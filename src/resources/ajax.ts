...
export async function signup(
  formData: models.LoginRequest
): Promise<models.User> {
  try {
    const {
      data: { jwt, user },
    } = await axios.post<models.LoginResponse>(
      api + apiRoutes.REGISTER,
      formData
    );
...
export async function getUser(id: number): Promise<models.User | null> {
  try {
    const { data } = await ajaxCaller().get<models.User>(
      apiRoutes.USER + "/" + id
    );
    return data;
  } catch (e) {
...
export async function editPassword(
  ...
  try {
    const { data } = await ajaxCaller().put<models.User>(
      apiRoutes.USER + "/" + id,
      formData
    );
    return data;
...
export async function getTaskById(id: number): Promise<models.Task | null> {
  try {
    const { data } = await ajaxCaller().get<models.Task>(
      apiRoutes.TASK + "/" + id
    );
    return data;
  } catch (e) {
...
export async function createTask(content: string): Promise<models.Task> {
  try {
    const { data } = await ajaxCaller().post<models.Task>(
      apiRoutes.TASK,
      content,
